...
vid = cv2.VideoCapture('/home/easemyai/Documents/object_seg/DPL_Sample_Video/Cam 192_168_1_17/20260317105131.ts')
image_count = 0
...
while True:
    ...
    if len(indices) > 0:
        for i in indices.flatten():
            ...
            # Save
            cv2.imwrite(os.path.join(OUTPUT_DIR, f"clean_{image_count}.jpg"), final_res)
            image_count += 1
            
            cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
            cv2.putText(frame, f"Conf: {nms_scores[i]:.2f}", (x1, y1-10), 0, 0.6, (0,255,0), 2)

    cv2.imshow("High-Accuracy ONNX", cv2.resize(frame, (1280, 720)))

...
def onnx_setup(onnx_path):
    """Loads the model and returns the session and input dimensions."""
    session = ort.InferenceSession(onnx_path, providers=['CPUExecutionProvider'])
    model_inputs = session.get_inputs()
    in_h, in_w = model_inputs[0].shape[2], model_inputs[0].shape[3]
    return session, model_inputs, in_h, in_w
...
def process_frame(frame, session, model_inputs, in_h, in_w):
    """Performs inference, NMS, and segmentation on a single frame."""
    CONF_THRESH = 0.65
    IOU_THRESH = 0.45
    ...
    for i in idx:
        ...
        nms_boxes.append([float(x1), float(y1), float(w/ratio), float(h/ratio)])
    ...
    if len(indices) > 0:
        for i in indices.flatten():
            ...
            # Segmentation Mask Assembly
            coeffs = nms_coeffs[i]
            c, mh, mw = protos.shape
            mask_raw = sigmoid((coeffs @ protos.reshape(c, -1)).reshape(mh, mw))
            
            # Re-scale mask back to original image
...
def run_video_processing(vid_path, onnx_path):
    """Handles the video loop and calling the processor."""
...
VIDEO_FILE = '/home/easemyai/Documents/object_seg/DPL_Sample_Video/Cam 192_168_1_17/20260317105131.ts'
MODEL_FILE = "/home/easemyai/Documents/object_seg/model/best.onnx"

run_video_processing(VIDEO_FILE, MODEL_FILE)